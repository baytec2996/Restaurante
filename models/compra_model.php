...
class Compra_Model extends Model
{
    ...
    {
        try {
            $stm = $this->db->prepare("SELECT id_tipo_ins,id_ins,ins_cod,ins_nom,ins_cat,ins_med FROM v_insprod WHERE (ins_cod LIKE '%$data%' OR ins_nom LIKE '%$data%') AND est_b = 'a' AND est_c = 'a' ORDER BY ins_nom");
    ...
    {
        try {
            $file = fopen("nuevo_proveedor.txt", "a+");
            fwrite($file, '|------HORA----------|----------DETALLE----------|' . PHP_EOL);

            fwrite($file, '[' . date("Y-m-d H:i:s") . '] ' . json_encode(array($data)) . PHP_EOL);
            fwrite($file, '|------------------------------------------------|' . PHP_EOL);
            fclose($file);

            $consulta = "call usp_comprasRegProveedor( :flag, 0, :ruc, :razon_social, :direccion, :telefono, :email, :contacto);";
            $arrayParam =  array(
                ':flag' => 1,
                ':ruc' => $data['ruc'],
            ...
            $file = fopen("error_productos.log", "a+");
            fwrite($file, '|------HORA----------|----------DETALLE----------|' . PHP_EOL);
            fwrite($file, '[' . date("Y-m-d H:i:s") . '] ' . $e . PHP_EOL);
            fwrite($file, '|------------------------------------------------|' . PHP_EOL);
            fclose($file);
