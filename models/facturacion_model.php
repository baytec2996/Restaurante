...
class Facturacion_Model extends Model
{
	...
	public function Datos1()
    {
        try
        {
            ...
            $stm = $this->db->prepare("SELECT v.*,IFNULL((v.total+v.comis_del-v.desc_monto),0) AS total FROM v_ventas_con AS v left JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.id_tdoc like ? AND v.id_tdoc <> 3 ".$estado." GROUP BY v.id_ven");
    ...
    public function Datos5()
    {
        ...
        {
            $ifecha = date('Y-m-d',strtotime($_POST['ifecha']));
            $stm = $this->db->prepare("SELECT fec_ven,desc_td,ser_doc,nro_doc,(total-desc_monto) AS total FROM v_ventas_con WHERE id_tdoc = 1 AND (enviado_sunat IS NULL OR enviado_sunat = '' OR enviado_sunat=0) AND DATE_FORMAT(fec_ven,'%Y-%m-%d') = ?");
