    ...
    */
    ...
    public function pc1($data)
    {
        try {
            ...
            $file = fopen("usp_restRegMesa.txt", "w");

            fwrite($file, "entro en usp_restRegMesa(data " . PHP_EOL);
    ...
    public function pc3($data)
    {
        try {
            ...
            $consulta = "call usp_restRegDelivery( :flag, :tipo_canal, :id_tipo_pedido, :id_apc, :id_usu, :fecha_pedido, :id_cliente, :id_repartidor, :tipo_entrega, :tipo_pago, :pedido_programado, :hora_entrega, :nombre_cliente, :telefono_cliente, :direccion_cliente, :referencia_cliente, :email_cliente,:estado);";
            $arrayParam =  array(
                ':flag' => 1,
                ':tipo_canal' => 1,
                ':id_tipo_pedido' => 3,
                ...
                ':estado' => 'a'
                //  ':estado' => 'b' --> con b llega defrente a preparacion no necesita confirmación  pero no tiene fecha y tampoco tiene comanda
            );
            $st = $this->db->prepare($consulta);
            $st->execute($arrayParam);
            $row = $st->fetch(PDO::FETCH_ASSOC);
    ...
    public function RegistrarPedido($data)
    {
        try {
            ...
            foreach ($data['items'] as $d) {
                ...
                $file = fopen("paso002.txt", "w");

                fwrite($file, "entro en INSERT INTO tm_detalle_pedido(data " . PHP_EOL);

                fwrite($file, json_encode(array($data['cod_ped'], $id_usu, $d['producto_id'], $d['cantidad'], $d['cantidad'], $d['precio'], $d['comentario'], $fecha, $estado)) . "aca sql->" . PHP_EOL);
    ...
    public function ListarDetallePed($data)
    {
        try {
            ...
            $Consulta = "SELECT d.id_pres,SUM(d.cantidad) AS cantidad, d.precio, d.estado FROM tm_detalle_pedido d inner join tm_producto_pres p on p.id_pres=d.id_pres WHERE id_pedido = " . $c->id_pedido . " AND d.estado <> 'z' GROUP BY d.id_pres, d.precio";





            $c->{'Detalle'} = $this->db->query($Consulta)
                ->fetchAll(PDO::FETCH_OBJ);
    ...
    */
    ...
    public function RegistrarVenta($data)
    {
        try {
            ...
            $apcid_ = 0;
            if (Session::get('apcid') > 0) {
                $apcid_ = Session::get('apcid');
            }
            ...
            $arrayParam = array(
                ...
                ':descuento_personal' => $descuento_personal_,
            ...
            $fp = fopen('usp_restEmitirVenta.txt', 'w');
            fwrite($fp, print_r(array_values($arrayParam), TRUE));
            fclose($fp);


            //$file = fopen("usp_restEmitirVenta.txt", "w");

            //fwrite($file, "entro en usp_restEmitirVenta( " . PHP_EOL);

            // fwrite($file, json_encode(array($arrayParam))."aca sql->" . PHP_EOL);
            //  fwrite($file, array($arrayParam)."aca sql->" . PHP_EOL);
