...
$(function() {
    ...
    .on('success.form.fv', function(e) {
        ...
        if ($("#cliente_id").val() == ''){
            ...
            if(Array.isArray(venta.idProd) && Array.isArray(venta.precProd) && Array.isArray(venta.cantProd)){ //==AGREGADO

                if(venta.idProd.length == venta.cantProd.length && venta.precProd.length == venta.cantProd.length && venta.idProd.length>0){

                    console.log('-------------------------------');
                    console.log('id:          '+ venta.idProd  );
                    console.log('cantidades : ' + venta.cantProd);
                    console.log('precios:     '+venta.precProd);
                    console.log('-------------------------------');
                   ...
                   //console.log(3==3);
                    ...
                    console.log('-------------------------------');
                    console.log('id:          '+ (Array.isArray(venta.idProd) && venta.idProd.length <0 )? venta.idProd :'EL objeto no es array o esta vacio');
                    console.log('cantidades : ' + venta.cantProd);
                    console.log('precios:     '+venta.precProd);
                    console.log('-------------------------------');
                    ...
                    var html_confirm = '<div>Error datos incompletos </div>\
                            <br>\
                            <div><span class="text-danger" style="font-size: 18px;">Actualice la pagina </span></div><br>\
                            <a href="'+$("#url").val()+'venta " class="btn btn-danger">Actualizar</a>';
...
var listarCategoriasMovil = function(){
   ...
   // $('#list-catgrs-movil').html('<a class="catP b-0 list-group-item list-group-item-action active" style="border-radius: 0px;" data-toggle="list" href="javascript:void(0)" onclick="listarProdsMasVend();"><b>FAVORITOS</b></a>');
    $.ajax({
        dataType: 'JSON',
        type: 'POST',
        data: {
        ...
        if($('#codimpcomandamesa').val() == 1){
            ...
            if(($('#codtipoped').val() == 1) || $('#codtipoped').val() == 2){
                ...
                for(i=0; i < array.length; i++){
                    console.log('Area es :');
                    console.log(parseInt(array[i].area_id));
...
var note_list = function(cod_add,cod_pres_add){
    ...
    $.ajax({
        ...
        $.each(arreglo2, function (ind, ele) {
            ...
            var element = document.getElementById(ele.trim());
